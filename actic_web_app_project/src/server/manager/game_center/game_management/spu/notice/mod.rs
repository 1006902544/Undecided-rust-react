...
pub async fn get_notice_limit(
    ...
    let sql_str = "select SQL_CALC_FOUND_ROWS * from spu_notation from where (spu_id=:spu_id or :spu_id is null) and (id=:id or :id is null) and (title=:title or :title is null) and (publish_type=:publish_type or :publish_type is null) and (published=:published or :published is null) order by update_time desc limit :scope,:limit";
